...
const EditProject = () => {
  ...
  // Extract project data from Redux store
  const { project, projectIsLoading, projectIsSuccessful } = useSelector(
    (state) => state.aProject
  );
  ...
  const [formData, setFormData] = useState({
    ...
    stack: project.stack || "",
    githubSource: project.githubSource || "",
    dockerSource: project.dockerSource || "",
    live: project.live || "",
  ...
  // Update `initialData` and sync `formData` when `project` changes
  useEffect(() => {
    if (project) {
      const updatedData = {
        title: project.title || "",
  ...
  // Separate state for the tag input
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(editProject({ projectsFormData, id }))
      .unwrap()
      .then((response) => {
